...
class MaxDiff(object):
    ...
    def __init__(self, model, samples=10, horizon=10, lam=0.1,
                 ...
                 receding=False,gamma=1.0, tensor=False, autoscale_alpha=False):
        ...
        if tensor:
            self.num_states  = self.num_states.item()   
        ...
        self.autoscale_alpha = autoscale_alpha
    ...
    def set_entropy_params(self,logdet_method='abs',print_params=False):
        ...
        if use_jit:
            ...
            self.get_entropy(torch.randn(self.t_H,self.samples,self.num_states,device=self.device)) 
        else:
        ...
        with torch.no_grad():
            ...
            if self.cost_to_go:
                ...
                if not eval:
                    ...
                    if self.autoscale_alpha:
                        S -= S.mean() # mean center
                        self.alpha = 0.5*torch.mean(sk.abs())/torch.mean(S.abs())
                        # self.alpha = 0.5*sk.max()/(S.max()-S.min())
                    sk = sk + self.alpha*S
                sk = sk - torch.max(sk)
